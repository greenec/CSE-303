 ...
 * threads.cc
 *
 * Threads demonstrates some of the ways that we can make threads and have them
 * interact.  This includes:
...
void run_timed_test(arg_t &args, std::function<void(int)> task) {
  ...
  // measured time.  As long as num_ints is big enough, that's OK.
 ...
 * each thread randomly chooses from among 1024 counters, and increments and
 * increments it.  Hence there should be parallelism, because threads aren't
 * usually operating on the same data at the same time.  We add two more
 * features.  The first is that we put each counter on its own cache line.  The
 * second is that we use std::atomic<> integers, to avoid locking.  Using a
 * std::atomic<> is safe because we are not doing anything more complex than a
 * fetch-and-add (via operator++).
 *
 * @param args The bundle of arguments to the program
 */
